  ...
  return (
    <>
      ...
      <Input
        ...
        value={textoBusca} onChange={event => setTextoBusca(event.target.value)} />

      <List>
        {pokemonList.filter(pokemon => pokemon.name.includes(textoBusca.toLowerCase()) ||
          String(pokemon.id) === textoBusca).map((pokemon, index) => { return (<CardPokemon key={index} id={pokemon.id} name={pokemon.name} types={pokemon.types} />) })}
      </List>
    </>
  )
}
